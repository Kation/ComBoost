...
namespace Wodsoft.ComBoost.EntityFramework.Test
{
    ...
    {
        private IDatabaseContext SeedData()
        {
            var dataContext = new DataContext("server=.;database=EntityFrameworkTest;Trusted_Connection=SSPI");
            if (dataContext.Tests.Count() == 0)
            {
                dataContext.Tests.Add(new TestEntity
                {
                    Id = Guid.NewGuid(),
        ...
        public async Task IncludeTest()
        {
            var databaseContext = SeedData();
            var entityContext = (EntityContext<TestEntity>)databaseContext.GetContext<TestEntity>();
            var items = await entityContext.Query().Include(t => t.Include).OrderBy(t => t.ValueInt).ThenBy(t => t.ValueDouble).ToArrayAsync();
